...
def formula2d():
    ...
    return Forall('$x', Forall('$y', Equiv(Grandmother('$x', '$y'), Exists('$p', AndList([Female('$y'), Parent('$x', '$p'), Parent('$p', '$y')])))))
    # END_YOUR_CODE
...
def liar():
    ...
    def only_one_true(A, B, formula):
        return Not(And(formula(A), formula(B)))

    def xor_list(persons, formula):
        if len(persons) == 2:
            return Xor(formula(persons[0]), formula(persons[1]))
        return Xor(formula(persons[0]), xor_list(persons[1:], formula))

    def max_one_true(persons, formula):
        x = []
        for i in range(len(persons)):
            j = i + 1
            while j < len(persons):
                x.append(only_one_true(persons[i], persons[j], formula))
                j += 1
        return AndList(x)

    persons = [mark, nicole, john, susan]
    formulas.append(And(xor_list(persons, TellTruth), max_one_true(persons, TellTruth)))
    formulas.append(And(xor_list(persons, CrashedServer), max_one_true(persons, CrashedServer)))
    # END_YOUR_CODE
