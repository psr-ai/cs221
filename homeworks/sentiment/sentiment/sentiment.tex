...
&= ((1 + e^{-w.\phi_x})^{-1} - y)^2
 ...
 \begin{align*}
 Loss_{w, x, y}&= \sigma(w\phi_{(x)}) - y && \textbf{...(1)}\\
 0 &= \sigma(w\phi_{(x)}) - y \\
...
\end{align*}
	...
	Thus, in our modified algorithm, the feature vectors will be consisting of two sets, one being $\phi_{x_i'}$ for every $x_{i'} \epsilon \{x_1, x_2, ... x_n\} and x_{i'} not in S$ and other being centroids of points of each pre defined clusters, $\{ \mu_{l1}^{k'}, \mu_{l2}^{k'}, ... \mu_{lK'}^{k'} \}$. Total length of points will be $n - d + K'$ (Total points minus number of points in S, added to number of pre defined clusters).
	...
	\end{align*}
	For remaining cluster centroids, $i' = 1, ... K'$ \\
	Assign $i'$ to cluster with closest centroid:
	\begin{align*}
	Z_{n-d+i'} & \leftarrow arg min_{k=1, ... K} || \mu_{li'}^{k'} - \mu_k ||
	\end{align*}
	\\
	\textbf{Step 2} \\
	...
	$$ \mu_k \leftarrow \frac{1}{|\{i:z = k\}|} \sum_{i:z_i=k} \phi(x_i)$$
