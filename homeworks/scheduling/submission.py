...
def create_chain_csp(n):
    ...
    for index, variable in enumerate(variables):
        ...
        if index is not 0:
            csp.add_binary_factor(variables[index - 1], variable, lambda val1, val2: 0 if val1 is val2 else 1)
    # END_YOUR_CODE
    return csp
            ...
            for var in self.csp.variables:
                if var not in assignment:
                    ...
                    for val in self.domains[var]:
                        if self.get_delta_weight(assignment, var, val) - 0.0 > 0.001:
                            var_count += 1
                    var_counts.append((var_count, var))
            return min(var_counts)[1]
            # END_YOUR_CODE
        ...
        for request in self.profile.requests:
            for quarter in self.profile.quarters:
                if len(request.quarters) > 0:
                    csp.add_unary_factor((request, quarter), lambda cid: (quarter in request.quarters) if cid else True)
        # END_YOUR_CODE
